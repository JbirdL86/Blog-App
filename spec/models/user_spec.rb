...
RSpec.describe User, type: :model do
  subject { User.new(name: 'Jbird', photo: 'some_photo', bio: 'some_bio', post_counter: 0) }
  ...
  describe 'invalid conditions' do
    ...
    it 'post_counter must be a integer' do
      subject.post_counter = 'sadasdsa'
      expect(subject).to_not be_valid
    end

    it 'post_counter must be a integer greater than 0 or equal to 0' do
  ...
  describe 'recent_posts' do
    it 'user.recents_posts => should retrieve 5 most recents posts' do
      user = User.create(name: 'Jbird', photo: 'some_photo', bio: 'some_bio', post_counter: 0)
      posts = []
      (1..10).each do |_i|
